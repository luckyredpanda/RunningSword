...
public abstract class MySwipeHelper extends ItemTouchHelper.SimpleCallback {
    ...
    private void drawButton(Canvas c, View ittemView, List<MyButton> buffer, int pos, float translationX){
        float right = ittemView.getRight();
        float dButtonWidth = -1*translationX / buffer.size();
        for (MyButton button:buffer){
            float left = right - dButtonWidth;
            button.onDraw(c, new RectF(left,ittemView.getTop(),right,ittemView.getBottom()),pos);
            right = left;
        }
    }
